...
export default function ClaimPage() {
  ...
  return (
  <div className="min-h-screen bg-gradient-to-br from-blue-900 via-blue-800 to-blue-950 text-white py-10 px-6" ref={topRef}>
    {/* SOUND TOGGLE */}
    <div className="fixed top-5 right-5 z-50">
      <button
        onClick={() => setSoundEnabled(!soundEnabled)}
        className="text-white bg-blue-900/40 hover:bg-blue-900/60 border border-yellow-400/40 px-3 py-1 rounded-full text-sm shadow"
      >
      ...
      {isConnected && (
        <AnimatePresence>
          <motion.div
            ...
            style={{
              background: 'linear-gradient(180deg, rgba(255,255,255,0.02), rgba(255,255,255,0.01))',
              boxShadow: '0 10px 30px rgba(0,0,0,0.6), 0 0 40px rgba(234, 179, 8, 0.08)',
              border: '1px solid rgba(255,255,255,0.04)',
              minHeight: 220,
            }}
          >
            <motion.div whileHover={{ rotateX: -4, rotateY: 4 }} transition={{ type: 'spring', stiffness: 200, damping: 14 }}>
              ...
              <label className="flex items-center space-x-2 mt-4">
                <input
                  ...
                  className="h-4 w-4 rounded border-gray-600 bg-black/40 accent-yellow-400"
                />
              ...
              <div className="mt-6">
                ...
                {eligibility === 'eligible' && (
                  <div className="text-center">
                    <p className="text-yellow-300 font-semibold mb-3">Ultra-lucky dawg, the gates is wide open!</p>
                    <motion.img
                      src={happyGif}
                      alt="Happy"
                ...
                {eligibility === 'ineligible' && (
                  <div className="text-center">
                    <p className="text-blue-300 font-semibold mb-3">😵 Dawg, no luck today. Maybe you should go back to 9-5.😂  Just kidding, catch us next time.</p>
                    <motion.img
                      src={sadGif}
                      alt="Sad"
        ...
        <AnimatePresence>
          ...
          >
            <motion.div whileHover={{ rotateX: -4, rotateY: -6 }} transition={{ type: 'spring', stiffness: 200, damping: 14 }}>
              <h3 className="text-yellow-300 font-extrabold text-lg">4. Claim Reward</h3>
              ...
              <div className="mt-6">
                {!claimed ? (
                  ...
                  <div className="text-center text-yellow-300 font-semibold">
                    <div> Dawg, stop refreshing! 🎉 Already claimed. You tryna take the whole supply? </div>
                    <div className="text-sm text-gray-300 mt-1"> Check your wallet, your tokens are already there.</div>
                  </div>
                )}
              </div>
            </motion.div>
      ...
      {showCelebration && (
        <>
          ...
          <motion.div
            ...
            transition={{ type: 'spring', stiffness: 260, damping: 20 }}
            className="fixed z-60 left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 bg-[#0b1220] border border-yellow-400/30 rounded-2xl shadow-2xl w-full max-w-lg p-6"
          >
            {/* confetti canvas */}
            ...
            <div className="mt-5 flex gap-3">
              <a
                ...
                style={{ background: 'linear-gradient(90deg,#facc15,#60a5fa)', color: '#fff' }}
              >
                View on Explorer
              </a>
              ...
              <button
                ...
                style={{ background: 'linear-gradient(90deg,#1d4ed8,#2563eb)', color: '#fff' }}
              >
                Close
              </button>
            </div>
